							...
							<div class="form-group col-md-6">
								...
								<select name="movimiento" class="form-control select2 muestraMensaje">
									...
									for(var i=0;i<resp3.length;i++){
										if(resp3[i].ES_VIGENTE==1 && resp3[i].VALOR=='CONTA'){
...
function eventosIngresosegresos(objeto){
    ...
    $('#'+objeto.tabla+' div').on( 'change','select',function(e){
	...
	try {
		...
		for(let a=0;a<concepto.length;a++){
			if(concepto[a].ES_VIGENTE==1){
				const select_concepto = new Option(concepto[a].CONCEPTO, concepto[a].ID_CONCEPTO, false, false);
	...
	confirm("¡"+verbo+" el registro: "+dato+"!",function(){
		...
		try {
			...
			if(resp.resultado){
				...
				let mov=(resp.info.ABREVIATURA=='INGR')?'primary':'danger';
